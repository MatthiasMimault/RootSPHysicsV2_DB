...
#define CheckErrorCuda(text)  __CheckErrorCuda(text,__FILE__,__LINE__)
	...
	//------------------------------------------------------------------------------
	/// Interaction of a particle with a set of particles. (Fluid/Float-Fluid/Float/Bound) with Pore pressure 3D - Lucas
	//------------------------------------------------------------------------------
	...
	template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> __device__ void KerInteractionForcesTest_M
	(bool boundp2, unsigned p1, const unsigned &pini, const unsigned &pfin, float visco
		, const float *ftomassp
		, const double2 *posxy, const double *posz, const float4 *pospress, const float4 *velrhop, const typecode *code, const unsigned *idp)
	{
	...
	//------------------------------------------------------------------------------
	/// Interaction of a particle with a set of particles. (Pore pressure, Pressure float, Mass, Solid) - Matthias
	//------------------------------------------------------------------------------
	template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> __device__ void KerInteractionForcesSolidBoxPPM_M
	(bool boundp2, unsigned p1, const unsigned &pini, const unsigned &pfin, float visco
		, const float *ftomassp
		, const double2 *posxy, const double *posz, const float4 *pospress, const float4 *velrhop, const typecode *code, const unsigned *idp
		, const float *mass, float ftmassp1, bool ftp1
		, double3 posdp1, float3 posp1, float3 velp1, float pressp1, float rhopp1
		, float3 &acep1, float &arp1, float &visc, float &deltap1
		, TpShifting tshifting, float3 &shiftposp1, float &shiftdetectp1		
...

	{
		for (int p2 = pini; p2 < pfin; p2++) {
			...
			if (rr2 <= CTE.fourh2 && rr2 >= ALMOSTZERO) {
				...
				bool ftp2;         //-Indicates if it is floating. | Indica si es floating.
				float ftmassp2;    //-Contains mass of floating body or massf if fluid. | Contiene masa de particula floating o massp2 si es bound o fluid.
				float massp2 = mass[p2];
				bool compute = true; //-Deactivated when DEM is used and is float-float or float-bound. | Se desactiva cuando se usa DEM y es float-float o float-bound.
				if (USE_FLOATING) {
					const typecode cod = code[p2];
					ftp2 = CODE_IsFloating(cod);
					ftmassp2 = (ftp2 ? ftomassp[CODE_GetTypeValue(cod)] : mass[p2]);
#ifdef DELTA_HEAVYFLOATING
					if (ftp2 && ftmassp2 <= (massp2*1.2f) && (tdelta == DELTA_Dynamic || tdelta == DELTA_DynamicExt))deltap1 = FLT_MAX;
#else
					if (ftp2 && (tdelta == DELTA_Dynamic || tdelta == DELTA_DynamicExt))deltap1 = FLT_MAX;
				...
				if (compute) {
					const tsymatrix3f prs = {
						(pressp1 + porep1 - taup1.xx + pressp2 + pore[p2] - tau[p2].xx) / (rhopp1*velrhop[p2].w) + (tker == KERNEL_Cubic ? KerGetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, pressp2) : 0),
						-(taup1.xy + tau[p2].xy) / (rhopp1*velrhop[p2].w),
						-(taup1.xz + tau[p2].xz) / (rhopp1*velrhop[p2].w),
						(pressp1 + porep1 - taup1.yy + pressp2 + pore[p2] - tau[p2].yy) / (rhopp1*velrhop[p2].w) + (tker == KERNEL_Cubic ? KerGetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, pressp2) : 0),
						-(taup1.yz + tau[p2].yz) / (rhopp1*velrhop[p2].w),
						(pressp1 + porep1 - taup1.zz + pressp2 + pore[p2] - tau[p2].zz) / (rhopp1*velrhop[p2].w) + (tker == KERNEL_Cubic ? KerGetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, pressp2) : 0)
					};
					const tsymatrix3f p_vpm3 = {
						-prs.xx*massp2*ftmassp1, -prs.xy*massp2*ftmassp1, -prs.xz*massp2*ftmassp1,
						-prs.yy*massp2*ftmassp1, -prs.yz*massp2*ftmassp1, -prs.zz*massp2*ftmassp1
	...
	//------------------------------------------------------------------------------
	/// Interaction of a particle with a set of particles. (Fluid/Float-Fluid/Float/Bound) with Pore pressure 1D WIP - Lucas
	//------------------------------------------------------------------------------
	template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> __device__ void KerInteractionForcesFluidBoxPPM
	(bool boundp2, unsigned p1, const unsigned &pini, const unsigned &pfin, float visco
		, const float *ftomassp, const float2 *tauff
		...
		if (p < n) {
			...
			for (int z = zini; z < zfin; z++) {
				...
				for (int y = yini; y < yfin; y++) {
					...
					/*if (pfin)KerInteractionForcesTest_M<psingle, tker, ftmode, lamsps, tdelta, shift>
						(false, p1, pini, pfin, viscof
							, ftomassp
							, posxy, posz, pospress, velrhop, code, idp);*/
					
					if (pfin)KerInteractionForcesSolidBoxPPM_M<psingle, tker, ftmode, lamsps, tdelta, shift>
						(false, p1, pini, pfin, viscof
							, ftomassp
							, posxy, posz, pospress, velrhop, code, idp
							, mass, ftmassp1, ftp1
							, posdp1, posp1, velp1, pressp1, rhopp1
							, acep1, arp1, visc, deltap1, tshifting, shiftposp1, shiftdetectp1
							, porep1, pore, taup1, tau, gradvelp1, omegap1);
			...
			for (int z = zini; z < zfin; z++) {
				...
				for (int y = yini; y < yfin; y++) {
					...
					KerInteractionForcesSolidBoxPPM_M<psingle, tker, ftmode, lamsps, tdelta, shift>
						(true, p1, pini, pfin, viscof
							, ftomassp
							, posxy, posz, pospress, velrhop, code, idp
							, mass, ftmassp1, ftp1
							, posdp1, posp1, velp1, pressp1, rhopp1
	...
	//##############################################################################
	//# Kernels for ComputeStep (vel & rhop) --Matthias--
	//# Kernels para ComputeStep (vel & rhop).
	/// Computes new values for Pos, Check, Vel and Ros (using Verlet).
	/// The value of Vel always set to be reset.
	//##############################################################################
	...
	//==============================================================================
	/// Compute of JauTauDot - Matthias
	//==============================================================================
	__global__ void KerComputeJauTauDot_M(unsigned n, unsigned pini, tsymatrix3f *taudot, tsymatrix3f *JauTauc2_M, tsymatrix3f *JauGradvelc2_M, tsymatrix3f *JauOmega_M)
	{
		unsigned p = blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x;
		if (p < n) {
			const tsymatrix3f tau = JauTauc2_M[p];
			...
			const tsymatrix3f E = {
				(2.0f / 3.0f*CTE.C1 - 1.0f / 3.0f*CTE.C12 - 1.0f / 3.0f*CTE.C13)*gradvel.xx + (2.0f / 3.0f*CTE.C12 - 1.0f / 3.0f*CTE.C2 - 1.0f / 3.0f*CTE.C23)*gradvel.yy + (2.0f / 3.0f*CTE.C13 - 1.0f / 3.0f*CTE.C23 - 1.0f / 3.0f*CTE.C3) * gradvel.zz,
				CTE.C4 * gradvel.xy,
				CTE.C5 * gradvel.xz,
				(-1.0f / 3.0f*CTE.C1 + 2.0f / 3.0f*CTE.C12 - 1.0f / 3.0f*CTE.C13)*gradvel.xx + (-1.0f / 3.0f*CTE.C12 + 2.0f / 3.0f*CTE.C2 - 1.0f / 3.0f*CTE.C23)*gradvel.yy + (-1.0f / 3.0f*CTE.C13 + 2.0f / 3.0f*CTE.C23 - 1.0f / 3.0f*CTE.C3)*gradvel.zz,
				CTE.C6 * gradvel.yz,
				(-1.0f / 3.0f*CTE.C1 - 1.0f / 3.0f*CTE.C12 + 2.0f / 3.0f*CTE.C13)*gradvel.xx + (-1.0f / 3.0f*CTE.C12 - 1.0f / 3.0f*CTE.C2 + 2.0f / 3.0f*CTE.C23)*gradvel.yy + (-1.0f / 3.0f*CTE.C13 - 1.0f / 3.0f*CTE.C23 + 2.0f / 3.0f*CTE.C3)*gradvel.zz };
	...
	void ComputeJauTauDot_M(unsigned np, unsigned npb, tsymatrix3f *taudot, tsymatrix3f *JauTau, tsymatrix3f *JauGradvel, tsymatrix3f *JauOmega)
	{
		const unsigned npf = np - npb;
		if (npf) {
			dim3 sgrid = cuSol::GetGridSize(npf, DIVBSIZE);
			KerComputeJauTauDot_M << <sgrid, DIVBSIZE >> > (npf, npb, taudot, JauTau, JauGradvel, JauOmega);
		}
	}

	//==============================================================================
	/// Interaction for the force computation. - Original
	...
	*/
		...
		if (npf) {
			...
			// Matthias
			cuSol::KerInteractionForcesSolid_M <psingle, tker, ftmode, lamsps, tdelta, shift> << <sgridf, bsfluid >> >
				(npf, npb, hdiv, nc, cellfluid, viscob, viscof
					, begincell, cellzero, dcell
					, ftomassp
					, posxy, posz, pospress, velrhop, code, idp
					, viscdt, ar, ace, delta, tshifting, shiftpos, shiftdetect
					, mass, pore, JauTau, JauGradvel, JauOmega);
			/*(unsigned n, unsigned pinit, int hdiv, int4 nc, unsigned cellfluid, float viscob, float viscof
			, const int2 *begincell, int3 cellzero, const unsigned *dcell
			, const float *ftomassp
			, const double2 *posxy, const double *posz, const float4 *pospress, const float4 *velrhop, const typecode *code, const unsigned *idp
		...
		// Compute Sdot
		ComputeJauTauDot_M(npf, npb, taudot, JauTau, JauGradvel, JauOmega);
	...
	//==============================================================================
	/// Surcharche solide de IntForceT - Matthias - Test compilation
	//==============================================================================
	template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void Interaction_ForcesT
	(TpCellMode cellmode, float viscob, float viscof, unsigned bsbound, unsigned bsfluid
		, unsigned np, unsigned npb, unsigned npbok, tuint3 ncells
	...
	//==============================================================================
	/// Surcharche solide de IntForceT - Lucas
	//==============================================================================
	template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void Interaction_ForcesT
	(TpCellMode cellmode, float viscob, float viscof, unsigned bsbound, unsigned bsfluid
		, unsigned np, unsigned npb, unsigned npbok, tuint3 ncells
	...
	//==============================================================================
	/// Selection of template parameters for Interaction_ForcesX. - Lucas
	...
	//==============================================================================
	/// Selection of template parameters for Interaction_Forces_M - Matthias
	/// Anisotropic and pressure 1D and Pore pressure
	//==============================================================================
	void Interaction_Forces_M(TpKernel TKernel, bool WithFloating, TpShifting TShifting, TpVisco TVisco, TpDeltaSph TDeltaSph, bool UseDEM
		, TpCellMode cellmode, float viscob, float viscof, unsigned bsbound, unsigned bsfluid
		, unsigned np, unsigned npb, unsigned npbok
		...
		if (TKernel == KERNEL_Wendland) {
			...
			if (!WithFloating) {
				...
				if (TShifting) {
					...
					if (TVisco == VISCO_LaminarSPS) {
						const bool lamsps = true;
						if (TDeltaSph == DELTA_None)      Interaction_ForcesT<psingle, tker, ftmode, lamsps, DELTA_None, tshift>(cellmode, viscob, viscof, bsbound, bsfluid, np, npb, npbok, ncells, begincell, cellmin, dcell, posxy, posz, pospress, velrhop, code, idp, ftomassp, jautau, jaugradvel, jautaudot, jauomega, viscdt, ar, ace, delta, TShifting, shiftpos, shiftdetect, simulate2d, kerinfo, bsauto, pore, mass);
						if (TDeltaSph == DELTA_Dynamic)   Interaction_ForcesT<psingle, tker, ftmode, lamsps, DELTA_Dynamic, tshift>(cellmode, viscob, viscof, bsbound, bsfluid, np, npb, npbok, ncells, begincell, cellmin, dcell, posxy, posz, pospress, velrhop, code, idp, ftomassp, jautau, jaugradvel, jautaudot, jauomega, viscdt, ar, ace, delta, TShifting, shiftpos, shiftdetect, simulate2d, kerinfo, bsauto, pore, mass);
						if (TDeltaSph == DELTA_DynamicExt)Interaction_ForcesT<psingle, tker, ftmode, lamsps, DELTA_DynamicExt, tshift>(cellmode, viscob, viscof, bsbound, bsfluid, np, npb, npbok, ncells, begincell, cellmin, dcell, posxy, posz, pospress, velrhop, code, idp, ftomassp, jautau, jaugradvel, jautaudot, jauomega, viscdt, ar, ace, delta, TShifting, shiftpos, shiftdetect, simulate2d, kerinfo, bsauto, pore, mass);
					}
					else {
	...
	, double2 *posxy, double *posz, float PoreZero, float *Poreg_M)
{
	...
	//==============================================================================
	/// Calculations for Press + porec // Matthias - Anisotropy
	//==============================================================================
		...
		, double2 *posxy, double *posz, float PoreZero, float *Porec_M)
	{
		if (np) {
			dim3 sgrid = cuSol::GetGridSize(np, SPHBSIZE);
			KerPressPore_M << <sgrid, SPHBSIZE >> > (np, velrhop, RhopZero, Pressg, CteB, Gamma, posxy, posz, PoreZero, Porec_M);
