...
#define CheckErrorCuda(text)  __CheckErrorCuda(text,__FILE__,__LINE__)
	...
	(unsigned n, unsigned npb, float rhopoutmin, float rhopoutmax
		...
		,float LambdaMass, float RhopZero,tmatrix3f *Ellipg, tmatrix3f *EllipDot)
	{
		unsigned p = blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
		if (p<n) {
			if (p<npb) { //-Particles: Fixed & Moving.
				...
				if (!floating || CODE_IsFluid(code[p])) { //-Particles: Fluid.
					...
					// update Ellip
					Ellipg[p].a11 = float(double(Ellipg[p].a11) + double(EllipDot[p].a11)*dt2);
					Ellipg[p].a12 = float(double(Ellipg[p].a12) + double(EllipDot[p].a12)*dt2);
					Ellipg[p].a13 = float(double(Ellipg[p].a13) + double(EllipDot[p].a13)*dt2);
					Ellipg[p].a21 = float(double(Ellipg[p].a21) + double(EllipDot[p].a21)*dt2);
					Ellipg[p].a22 = float(double(Ellipg[p].a22) + double(EllipDot[p].a22)*dt2);
					Ellipg[p].a23 = float(double(Ellipg[p].a23) + double(EllipDot[p].a23)*dt2);
					Ellipg[p].a31 = float(double(Ellipg[p].a31) + double(EllipDot[p].a31)*dt2);
					Ellipg[p].a32 = float(double(Ellipg[p].a32) + double(EllipDot[p].a32)*dt2);
					Ellipg[p].a33 = float(double(Ellipg[p].a33) + double(EllipDot[p].a33)*dt2);
	...
	void ComputeStepVerlet_L(bool floating, bool shift, unsigned np, unsigned npb, const float4 * velrhop1, const float4 * velrhop2, const float * ar, const float3 * ace, const float3 * shiftpos, double dt, double dt2, float rhopoutmin, float rhopoutmax, typecode * code, double2 * movxy, double * movz, float4 * velrhopnew, const tsymatrix3f * tau2, tsymatrix3f * JauTauDot_M, tsymatrix3f * taunew, const float * mass1, const float * mass2, float * massnew,float LambdaMass, float RhopZero,tmatrix3f *Ellipg,tmatrix3f *Ellipdot)
	{
		...
		if (np) {
			...
			if (shift) {
				...
				if (floating)cuSol::KerComputeStepVerlet_L<true, shift> << <sgrid, SPHBSIZE >> > (np, npb, rhopoutmin, rhopoutmax, velrhop1, velrhop2, ar, ace, shiftpos, dt, dt205, dt2, movxy, movz, code, velrhopnew, tau2, JauTauDot_M, taunew,mass1, mass2, massnew,LambdaMass,RhopZero, Ellipg, Ellipdot);
				else         cuSol::KerComputeStepVerlet_L<false, shift> << <sgrid, SPHBSIZE >> > (np, npb, rhopoutmin, rhopoutmax, velrhop1, velrhop2, ar, ace, shiftpos, dt, dt205, dt2, movxy, movz, code, velrhopnew, tau2, JauTauDot_M, taunew,mass1, mass2, massnew, LambdaMass, RhopZero, Ellipg, Ellipdot);
			}
			else {
...
__global__ void KerComputeJauEllips_L(unsigned n, unsigned pini, tmatrix3f *Jaugradu_T, tmatrix3f *JauEllipg, tmatrix3f *JauEllipDot)
{
	unsigned p = blockIdx.y*gridDim.x*blockDim.x + blockIdx.x*blockDim.x + threadIdx.x;
	if (p < n) {
		const tmatrix3f gradu = Jaugradu_T[p];
		const tmatrix3f ellipg = JauEllipg[p];
		JauEllipDot[p].a11 = ellipg.a11 * gradu.a11 + ellipg.a12 * gradu.a21 + ellipg.a13 * gradu.a31;
		JauEllipDot[p].a12 = ellipg.a11 * gradu.a12 + ellipg.a12 * gradu.a22 + ellipg.a13 * gradu.a32;
		JauEllipDot[p].a13 = ellipg.a11 * gradu.a13 + ellipg.a12 * gradu.a23 + ellipg.a13 * gradu.a33;
		JauEllipDot[p].a21 = ellipg.a21 * gradu.a11 + ellipg.a22 * gradu.a21 + ellipg.a23 * gradu.a31;
		JauEllipDot[p].a22 = ellipg.a21 * gradu.a12 + ellipg.a22 * gradu.a22 + ellipg.a23 * gradu.a32;
		JauEllipDot[p].a23 = ellipg.a21 * gradu.a13 + ellipg.a22 * gradu.a23 + ellipg.a23 * gradu.a33;
		JauEllipDot[p].a31 = ellipg.a31 * gradu.a11 + ellipg.a32 * gradu.a21 + ellipg.a33 * gradu.a31;
		JauEllipDot[p].a32 = ellipg.a31 * gradu.a12 + ellipg.a32 * gradu.a22 + ellipg.a33 * gradu.a32;
		JauEllipDot[p].a33 = ellipg.a31 * gradu.a13 + ellipg.a32 * gradu.a23 + ellipg.a33 * gradu.a33;
...
void ComputeJauEllips_L(unsigned np, unsigned npb, tmatrix3f *Jaugradu_T, tmatrix3f *JauEllipg, tmatrix3f *JauEllipDot)
{
	...
	if (npf) {
		dim3 sgrid = cuSol::GetGridSize(npf, DIVBSIZE);
		KerComputeJauEllips_L << <sgrid, DIVBSIZE >> > (npf, npb, Jaugradu_T, JauEllipg, JauEllipDot);
	}
}
...
(TpCellMode cellmode, float viscob, float viscof, unsigned bsbound, unsigned bsfluid
	...
	if (npf) {
		dim3 sgridf = cuSol::GetGridSize(npf, bsfluid);
		cuSol::KerInteractionForcesSolMass <psingle, tker, ftmode, lamsps, tdelta, shift> << <sgridf, bsfluid >> > (npf, npb, hdiv, nc, cellfluid, 0, viscof, begincell, cellzero, dcell, JauTau, JauGradvel, JauOmega, ftomassp, (const float2*)tau, (float2*)gradvel, posxy, posz, pospress, velrhop, code, idp, press, pore, mass, viscdt, ar, ace, delta, gradu_T, tshifting, shiftpos, shiftdetect);
		cuSol::KerInteractionForcesSolMass <psingle, tker, ftmode, lamsps, tdelta, shift> << <sgridf, bsfluid >> > (npf, npb, hdiv, nc, 0, viscob, 0, begincell, cellzero, dcell, JauTau, JauGradvel, JauOmega, ftomassp, (const float2*)tau, (float2*)gradvel, posxy, posz, pospress, velrhop, code, idp, press, pore, mass, viscdt, ar, ace, delta, gradu_T, tshifting, shiftpos, shiftdetect);
	}
		// Compute Sdot
	ComputeJauTauDot(npf, npb, taudot, JauTau, JauGradvel, JauOmega, AnisotropyG_M, Mu);

		// Compute Ellipse
	ComputeJauEllips_L(np, npb, gradu_T, ellipg, ellipDot);
		//-Interaction Boundary-Fluid.
	if (npbok) {
		dim3 sgridb = cuSol::GetGridSize(npbok, bsbound);
		cuSol::KerInteractionForcesBound<psingle, tker, ftmode> << <sgridb, bsbound >> > (npbok, hdiv, nc, begincell, cellzero, dcell, ftomassp, posxy, posz, pospress, velrhop, code, idp, viscdt, ar);
	...
	if (TKernel == KERNEL_Wendland) {
		...
		if (!WithFloating) {
			...
			if (TShifting) {
				...
				if (TVisco == VISCO_LaminarSPS) {
					const bool lamsps = true;
					if (TDeltaSph == DELTA_None)      Interaction_ForcesT<psingle, tker, ftmode, lamsps, DELTA_None, tshift>(cellmode, viscob, viscof, bsbound, bsfluid, np, npb, npbok, ncells, begincell, cellmin, dcell, posxy, posz, pospress, velrhop, code, idp,ftomassp, tau, gradvel, omega, jautau, jaugradvel, jautaudot, jauomega, AnisotropyG, Mu, viscdt, ar, ace, delta, gradu_T,ellipg, ellipDot,TShifting, shiftpos, shiftdetect, simulate2d, kerinfo, bsauto, press, pore, mass);
					if (TDeltaSph == DELTA_Dynamic)   Interaction_ForcesT<psingle, tker, ftmode, lamsps, DELTA_Dynamic, tshift>(cellmode, viscob, viscof, bsbound, bsfluid, np, npb, npbok, ncells, begincell, cellmin, dcell, posxy, posz, pospress, velrhop, code, idp,ftomassp, tau, gradvel, omega, jautau, jaugradvel, jautaudot, jauomega, AnisotropyG, Mu, viscdt, ar, ace, delta, gradu_T,ellipg, ellipDot,TShifting, shiftpos, shiftdetect, simulate2d, kerinfo, bsauto, press, pore, mass);
					if (TDeltaSph == DELTA_DynamicExt)Interaction_ForcesT<psingle, tker, ftmode, lamsps, DELTA_DynamicExt, tshift>(cellmode, viscob, viscof, bsbound, bsfluid, np, npb, npbok, ncells, begincell, cellmin, dcell, posxy, posz, pospress, velrhop, code, idp,ftomassp, tau, gradvel, omega, jautau, jaugradvel, jautaudot, jauomega, AnisotropyG, Mu, viscdt, ar, ace, delta, gradu_T,ellipg, ellipDot,TShifting, shiftpos, shiftdetect, simulate2d, kerinfo, bsauto, press, pore, mass);
				}
				else {
