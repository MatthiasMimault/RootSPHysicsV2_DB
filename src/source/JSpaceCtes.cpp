...
void JSpaceCtes::ReadAddXmlRun_M(JXml *sxml, TiXmlElement* node) {
	...
	SetLambdaMass(sxml->ReadElementDouble(node, "lambdamass", "value"));
	SetSizeDivision(sxml->ReadElementDouble(node, "sizedivision", "value"));
	...
	SetAM0(sxml->ReadElementFloat(node, "typeYoung", "position"));
	SetAM0(sxml->ReadElementFloat(node, "typeYoung", "stiffness"));
...
/// Loads constants for execution of the case of xml node.
//==============================================================================
void JSpaceCtes::LoadAddXmlRun_M(JXml *sxml, const std::string &place) {
	TiXmlNode* node = sxml->GetNode(place, false);
	if (!node)RunException("LoadAddXmlRun_M", std::string("The item is not found \'") + place + "\'.");
	ReadAddXmlRun_M(sxml, node->ToElement());
}
