...
#pragma omp parallel for schedule (guided)
	...
	for (int p1 = int(pinit); p1 < pfin; p1++) {
		...
		L[p1] = Inv3f(Mp1);
		//L[p1] = { 1, 0, 0, 0, 1, 0, 0, 0, 1 };
		/*printf("MP1 %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n", Mp1.a11, Mp1.a12, Mp1.a13, Mp1.a21
			, Mp1.a22, Mp1.a23, Mp1.a31, Mp1.a32, Mp1.a33);
		printf("L %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f %.8f\n", L[p1].a11, L[p1].a12, L[p1].a13, L[p1].a21, L[p1].a22, L[p1].a23, L[p1].a31, L[p1].a32, L[p1].a33);
		cin.get();*/
	}
}
...
//==============================================================================
/// Interaction particles with P/r2+P/r2 formulation - Matthias
//==============================================================================
...
//==============================================================================
/// Interaction particles v1.3 - CorrectL with Pi Visco - Matthias
//==============================================================================
...
#pragma omp parallel for schedule (guided)
	...
	for (int p1 = int(pinit); p1 < pfin; p1++) {
				...
				for (unsigned p2 = pini; p2 < pfin; p2++) {
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						if (compute) {
							...
							/*if (idp[p1] == 500) {
							//if (idp[p1] == 17415) {
								printf("Id: %d - Pressp1: %.8f - Pressp2: %.8f - frx: %.8f - massp2 %.8f - Lp1 %.8f\n", Idpc[p1], pressp1, press[p2], frx, massp2, L[p1].a11);
								printf("Id: %d - Prs: %.8f - P_vpm3: %.8f - Acep1.x: %.8f - Exact %.8f\n", Idpc[p1], prs.xx, p_vpm3.xx, acep1.x, RhopZero*Posc[p1].x);
								//cin.get();
							}*/
						}
		...
		if (shift || arp1 || acep1.x || acep1.y || acep1.z || visc) {
			...
			/*if (abs(posp1.x)<0.2 ) {
				//if (idp[p1] == 17415) {
				printf("Id: %d - Acep1.x: %.8f - Exact %.8f\n", Idpc[p1], ace[p1].x, RhopZero*Posc[p1].x);
				//cin.get();
			}*/
			const int th = omp_get_thread_num();
			if (visc > viscth[th*OMP_STRIDE])viscth[th*OMP_STRIDE] = visc;
