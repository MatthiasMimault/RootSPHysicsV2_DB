...
//////////////////////////////////////
// Surcharge of GetParticlesData_M with Nabvx -- Matthias
// #SigCst
//////////////////////////////////////
unsigned JSphSolidCpu::GetParticlesData_M(unsigned n, unsigned pini, bool cellorderdecode, bool onlynormal
...
// Interaction Bound-Fluid with Tau computation
template<bool psingle, TpKernel tker, TpFtMode ftmode> void JSphSolidCpu::InteractionForcesBound11_M
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop, const typecode *code, const unsigned *idp
...
#pragma omp parallel for schedule (guided)
	...
	for (int p1 = int(pinit); p1 < pfin; p1++) {
		...
		if (arp1 || visc || gradvelp1.xx || gradvelp1.xy || gradvelp1.xz || gradvelp1.yy || gradvelp1.yz || gradvelp1.zz
			|| omegap1.xx || omegap1.xy || omegap1.xz || omegap1.yy || omegap1.yz || omegap1.zz) {
			ar[p1] += arp1;
			const int th = omp_get_thread_num();
			if (visc > viscth[th*OMP_STRIDE])viscth[th*OMP_STRIDE] = visc;
...
//==============================================================================
/// Interaction particles with SMQ with NSPH correction - Matthias Deprecated
//==============================================================================
...
#pragma omp parallel for schedule (guided)
	...
	for (int p1 = int(pinit); p1<pfin; p1++) {
		...
		/*if (p1 == 1350) {
			printf("P11350: %.8f", pressp1, porep1, )
		}*/
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						if (true) {
							if (!ftp1) {//-When p1 is a fluid particle / Cuando p1 es fluido. 
								...
								if (Part > 220 && idp[p1] == 927 && idp[p2] < 500) printf("%d - %.8f %.8f %.8f\n", idp[p1], volp2, massp2, velrhop[p2].w);
							}
						}
					}
				}
...
//==============================================================================
/// Correction computation L - Matthias
// #Nsph
//==============================================================================
template<bool psingle, TpKernel tker> void JSphSolidCpu::ComputeNsphCorrection
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
...
template<bool psingle, TpKernel tker> void JSphSolidCpu::ComputeNsphCorrection11
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop
	, const float *mass, tmatrix3f *L)const
...
// With 2D version
template<bool psingle, TpKernel tker> void JSphSolidCpu::ComputeNsphCorrection12
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop
	, const float *mass, tmatrix3f *L)const
...
template<bool psingle, TpKernel tker> void JSphSolidCpu::ComputeNsphCorrectionX
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop
	, const float *mass, tmatrix3f *L)const
...
#pragma omp parallel for schedule (guided)
	...
	for (int p1 = int(pinit); p1 < pfin; p1++) {
		...
		// Matthias
		tmatrix3f Mp1 = { 1, 0, 0, 0, 1, 0, 0, 0, 1 };
...
// #Correction 
// V1.3 with 2D/3D debug (Corner sinks)
template<bool psingle, TpKernel tker> void JSphSolidCpu::ComputeNsphCorrection13
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial
	, const unsigned* beginendcell, tint3 cellzero, const unsigned* dcell
	, const tdouble3* pos, const tfloat3* pspos, const tfloat4* velrhop
	, const float* mass, tmatrix3f* L)const
{
	//printf("Start NSPH Correction\n");
	const bool boundp2 = (!cellinitial); //-Interaction with type boundary (Bound). | Interaccion con Bound.
	float viscth[OMP_MAXTHREADS * OMP_STRIDE];
	for (int th = 0; th < OmpThreads; th++)viscth[th * OMP_STRIDE] = 0;
	//-Initialise execution with OpenMP. | Inicia ejecucion con OpenMP..
...
#pragma omp parallel for schedule (guided)
	...
	for (int p1 = int(pinit); p1 < pfin; p1++) {
		/*printf(" P1 %d %.4f %.4f %.4f\n", Idpc[p1], Posc[p1].x, Posc[p1].y, Posc[p1].z);
		printf(" Nc %d %d %d %d\n", nc.x, nc.y, nc.z, nc.w);*/
			...
			for (int y = yini; y < yfin; y++) {
				...
				for (unsigned p2 = pini; p2 < pfin; p2++) {
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						//printf("P2 %d (%.4f, %.4f, %.4f) - F(%.4f, %.4f, %.4f)", Idpc[p2], drx, dry, drz, frx, fry, frz);
			...
			for (int y = yini; y < yfin; y++) {
				...
				for (unsigned p2 = pini; p2 < pfin; p2++) {
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						//printf("P2 %d (%.4f, %.4f, %.4f) - F(%.4f, %.4f, %.4f\n)", Idpc[p2], drx, dry, drz, frx, fry, frz);
...
//==============================================================================
/// Interaction particles with SMQ with L, the correction V1.1 - Matthias
//==============================================================================
template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void JSphSolidCpu::InteractionForcesNSPH11_M
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial, float visco
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tsymatrix3f* tau, tsymatrix3f* gradvel, tsymatrix3f* omega
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop, const typecode *code, const unsigned *idp
...
//==============================================================================
/// Interaction particles with P/r2+P/r2 formulation - Matthias - Deprecated
//==============================================================================
template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void JSphSolidCpu::InteractionForcesNSPH12_M
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial, float visco
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tsymatrix3f* tau, tsymatrix3f* gradvel, tsymatrix3f* omega
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop, const typecode *code, const unsigned *idp
			...
			for (int y = yini; y < yfin; y++) {
				...
				for (unsigned p2 = pini; p2 < pfin; p2++) {
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						if (compute) {
							const tsymatrix3f prs = {
								(pressp1 + porep1 - taup1.xx) / (rhopp1*rhopp1) + (press[p2] + pore[p2] - tau[p2].xx) / (velrhop[p2].w*velrhop[p2].w) + (tker == KERNEL_Cubic ? GetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, press[p2]) : 0),
								-(taup1.xy / (rhopp1*rhopp1) + tau[p2].xy / (velrhop[p2].w*velrhop[p2].w)),
								-(taup1.xz / (rhopp1*rhopp1) + tau[p2].xz / (velrhop[p2].w*velrhop[p2].w)),
								(pressp1 + porep1 - taup1.yy) / (rhopp1*rhopp1) + (press[p2] + pore[p2] - tau[p2].yy) / (velrhop[p2].w*velrhop[p2].w) + (tker == KERNEL_Cubic ? GetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, press[p2]) : 0),
								-(taup1.yz / (rhopp1*rhopp1) + tau[p2].yz / (velrhop[p2].w*velrhop[p2].w)),
								(pressp1 + porep1 - taup1.zz) / (rhopp1*rhopp1) + (press[p2] + pore[p2] - tau[p2].zz) / (velrhop[p2].w*velrhop[p2].w) + (tker == KERNEL_Cubic ? GetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, press[p2]) : 0)
							};
							const tsymatrix3f p_vpm3 = {
								-prs.xx*massp2*ftmassp1, -prs.xy*massp2*ftmassp1, -prs.xz*massp2*ftmassp1,
								-prs.yy*massp2*ftmassp1, -prs.yz*massp2*ftmassp1, -prs.zz*massp2*ftmassp1
...
//==============================================================================
/// Interaction particles v1.3 - CorrectL with Pi Visco - Matthias - Deprecated
//==============================================================================
template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void JSphSolidCpu::InteractionForcesNSPH13_M
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial, float visco
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tsymatrix3f* tau, tsymatrix3f* gradvel, tsymatrix3f* omega
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop, const typecode *code, const unsigned *idp
						...
						if (compute) {
							...
							if (!lamsps) {//-Artificial viscosity.
								if (dot < 0) {
									...
									const float robar = (rhopp1 + velrhop[p2].w)*0.5f;
									const float pi_visc = (-visco * cbar*amubar / robar)*massp2*ftmassp1;
									acep1.x -= pi_visc * frx*L[p1].a11; acep1.y -= pi_visc * fry*L[p1].a22; acep1.z -= pi_visc * frz*L[p1].a33;
								}
							}
						}
...
//==============================================================================
/// Special reformulation with VelCst
//==============================================================================
template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void JSphSolidCpu::InteractionForces_VelCst_M
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial, float visco
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tsymatrix3f* tau, tsymatrix3f* gradvel, tsymatrix3f* omega
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop, const typecode *code, const unsigned *idp
			...
			for (int y = yini; y < yfin; y++) {
				...
				for (unsigned p2 = pini; p2 < pfin; p2++) {
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						//===== Acceleration ===== 
						// Constant vel => null acceleration
						/*if (compute) {
							const tsymatrix3f prs = {
								(pressp1 + porep1 - taup1.xx + press[p2] + pore[p2] - tau[p2].xx) / (rhopp1*velrhop[p2].w) + (tker == KERNEL_Cubic ? GetKernelCubicTensil(rr2, rhopp1, pressp1, velrhop[p2].w, press[p2]) : 0),
								-(taup1.xy + tau[p2].xy) / (rhopp1*velrhop[p2].w),
...
//==============================================================================
/// Version with constant quadratic pressure field
//==============================================================================
// #GradVel #Sigma
template<bool psingle, TpKernel tker, TpFtMode ftmode, bool lamsps, TpDeltaSph tdelta, bool shift> void JSphSolidCpu::InteractionForces_CstSig_M
(unsigned n, unsigned pinit, tint4 nc, int hdiv, unsigned cellinitial, float visco
	, const unsigned *beginendcell, tint3 cellzero, const unsigned *dcell
	, const tsymatrix3f* tau, tsymatrix3f* gradvel, tsymatrix3f* omega
	, const tdouble3 *pos, const tfloat3 *pspos, const tfloat4 *velrhop, const typecode *code, const unsigned *idp
...
//==============================================================================
/// Interaction particles V11 with print Grad P - Matthias
//==============================================================================
			...
			for (int y = yini; y<yfin; y++) {
				...
				for (unsigned p2 = pini; p2<pfin; p2++) {
					...
					if (rr2 <= Fourh2 && rr2 >= ALMOSTZERO) {
						...
						const float dvx = velp1.x - velrhop[p2].x, dvy = velp1.y - velrhop[p2].y, dvz = velp1.z - velrhop[p2].z;
						if (compute)arp1 += massp2 * (dvx*frx + dvy * fry + dvz * frz);
						//if (Part > 57 && idp[p1] == 866) printf("arp1: %.8f\n", arp1);
						//printf("arp1: %.8f\n", arp1);
	...
	, TpShifting tshifting, tfloat3 *shiftpos, float *shiftdetect)const
{
	...
	if (npbok) {
		//-Interaction Bound-Fluid.
		InteractionForcesBound11_M<psingle, tker, ftmode>(npbok, 0, nc, hdiv, cellfluid, begincell, cellzero, dcell, pos, pspos, velrhop, code, idp, viscdt
...
void JSphSolidCpu::ComputeSymplecticPre_M(double dt) {
	bool deve = true;
	if (deve) {
		if (TShifting)ComputeSymplecticPreT_BlockBdy_M<false>(dt); //-We strongly recommend running the shifting correction only for the corrector. 
		else         ComputeSymplecticPreT_BlockBdy_M<false>(dt); // If you want to re-enable shifting in the predictor, change the value here to "true".
	}
	else {
...
void JSphSolidCpu::ComputeSymplecticPre_VelCst_M(double dt) {
	if (TShifting)ComputeSymplecticPreVcT_M<false>(dt); //-We strongly recommend running the shifting correction only for the corrector. 
	else         ComputeSymplecticPreVcT_M<false>(dt); // If you want to re-enable shifting in the predictor, change the value here to "true".	
}

template<bool shift> void JSphSolidCpu::ComputeSymplecticPreVcT_M(double dt) {
	TmcStart(Timers, TMC_SuComputeStep);
	//-Assign memory to variables Pre. | Asigna memoria a variables Pre.
	PosPrec = ArraysCpu->ReserveDouble3();
	VelrhopPrec = ArraysCpu->ReserveFloat4();
...
void JSphSolidCpu::ComputeSymplecticCorr_VelCst_M(double dt) {
	if (TShifting)ComputeSymplecticCorrVcT_M<true>(dt);
	else         ComputeSymplecticCorrVcT_M<false>(dt);
}
...
#pragma omp parallel for schedule (static) if(np>OMP_LIMIT_COMPUTESTEP)
	...
	for (int p = npb; p < np; p++) {
		...
		if (!WithFloating || CODE_IsFluid(Codec[p])) {//-Fluid Particles.
			...
			Velrhopc[p].w = rhopnew;
			//(Nstep == 200 || Nstep == 250 || Nstep == 300)
			/*if ((Part == 25 || Part == 50 || Part == 75 || Part == 100) && abs(Posc[p].y) < 0.05 && abs(Posc[p].z) < 0.05) {
				printf("CenterId %d Posx %.8f Vx %.8f Gvx %.8f (Part %d - Nsteps %d)\n", Idpc[p]
					, Posc[p].x, Velrhopc[p].x, GdVel.a11, Part, Nstep);
			}
			if ((Part == 25 || Part == 50 || Part == 75 || Part == 100) && Posc[p].y > 0.9 && Posc[p].z > 0.9) {
				printf("BorderId %d Posx %.8f Vx %.8f Gvx %.8f (Part %d - Nsteps %d)\n", Idpc[p]
					, Posc[p].x, Velrhopc[p].x, GdVel.a11, Part, Nstep);
			}*/
		}
		else {//-Floating Particles.
...
/// /////////////////////////////////////////////
// Code symplectic version 
// with T19 compression of 0.3*0.1*0.1 root
/// /////////////////////////////////////////////

void JSphSolidCpu::ComputeSymplecticPre_T19(double dt) {
	if (TShifting)ComputeSymplecticPreT_T19<false>(dt); //-We strongly recommend running the shifting correction only for the corrector. 
	else         ComputeSymplecticPreT_T19<false>(dt); // If you want to re-enable shifting in the predictor, change the value here to "true".	
}

template<bool shift> void JSphSolidCpu::ComputeSymplecticPreT_T19(double dt) {
	TmcStart(Timers, TMC_SuComputeStep);
	//-Assign memory to variables Pre. | Asigna memoria a variables Pre.
	PosPrec = ArraysCpu->ReserveDouble3();
	VelrhopPrec = ArraysCpu->ReserveFloat4();
...
void JSphSolidCpu::ComputeSymplecticCorr_T19(double dt) {
	if (TShifting)ComputeSymplecticCorrT_T19<true>(dt);
	else         ComputeSymplecticCorrT_T19<false>(dt);
}

template<bool shift> void JSphSolidCpu::ComputeSymplecticCorrT_T19(double dt) {
	TmcStart(Timers, TMC_SuComputeStep);
	...
	for (int p = npb; p < np; p++) {
		...
		if (Posc[p].x > -0.12)
		{
			...
			if (!WithFloating || CODE_IsFluid(Codec[p])) {//-Fluid Particles.
				...
				// compress the tip (px>0.12-vcompress*T)
				if (Posc[p].x > 0.12 + velcompress * TimeStep && TimeStep < 200.0){
					Velrhopc[p].x = velcompress;
					Velrhopc[p].y = 0;
					Velrhopc[p].z = 0;
				}
				else if (Posc[p].x > 0.12 + velcompress * 200.0 && TimeStep >= 200.0) {
					Velrhopc[p].x = 0;
					Velrhopc[p].y = 0;
					Velrhopc[p].z = 0;
				}
				// move cells inside [-0.12 0.12-vT]
				else if (Posc[p].x > -0.12 && Posc[p].x <= 0.12) {
					Velrhopc[p].x = float(double(VelrhopPrec[p].x) + double(Acec[p].x) * dt);
					Velrhopc[p].y = float(double(VelrhopPrec[p].y) + double(Acec[p].y) * dt);
					Velrhopc[p].z = float(double(VelrhopPrec[p].z) + double(Acec[p].z) * dt);
				}
				// static side of the root (px<-0.12)
...
/// /////////////////////////////////////////////
// Code version with sigma=1/3x^2
// Approximation evaluation
/// /////////////////////////////////////////////
void JSphSolidCpu::ComputeSymplecticPre_SigCst_M(double dt) {
	if (TShifting)ComputeSymplecticPreT_SigCst_M<false>(dt); //-We strongly recommend running the shifting correction only for the corrector. 
	else         ComputeSymplecticPreT_SigCst_M<false>(dt); // If you want to re-enable shifting in the predictor, change the value here to "true".	

}

template<bool shift> void JSphSolidCpu::ComputeSymplecticPreT_SigCst_M(double dt) {
	TmcStart(Timers, TMC_SuComputeStep);
	//-Assign memory to variables Pre. | Asigna memoria a variables Pre.
	PosPrec = ArraysCpu->ReserveDouble3();
	VelrhopPrec = ArraysCpu->ReserveFloat4();
...
#pragma omp parallel for schedule (static) if(np>OMP_LIMIT_COMPUTESTEP)
	...
	for (int p = npb; p < np; p++) {
		//-Calculate density.
		//const float rhopnew = float(double(VelrhopPrec[p].w) + dt05 * Arc[p]); // Not const because of source update 
		const float rhopnew = RhopZero * exp(float(-0.5*TimeStep*TimeStep));
		...
		if (!WithFloating || CODE_IsFluid(Codec[p])) {//-Fluid Particles.
			...
			if ((Idpc[p] < 1764 || Idpc[p] > 16316) ) {
				Velrhopc[p].x = float(TimeStep * Posc[p].x);
			}
			// #Grad #Gx #SigCst	
			/*Velrhopc[p].x = float(Posc[p].x*TimeStep);
			Velrhopc[p].y = 0.0f;
			Velrhopc[p].z = 0.0f;*/
			...
			tmatrix3f DQD = ToTMatrix3f((TMatrix3d(1, 0, 0, 0, 1, 0, 0, 0, 1) - dt05
				* ToTMatrix3d(Ttransp(GdVel))) * ToTMatrix3d(Q) * (TMatrix3d(1, 0, 0, 0, 1, 0, 0, 0, 1) - dt05 * ToTMatrix3d(GdVel)));
			//27/03/19 - Is it possible to reduce this DQD line ?
			QuadFormc_M[p].xx = float(DQD.a11);
			QuadFormc_M[p].xy = float(DQD.a12);
			QuadFormc_M[p].xz = float(DQD.a13);
			QuadFormc_M[p].yy = float(DQD.a22);
			...
			// Source Density and Mass - Commented since Source/Mass only in CorrT
			//27/03/19 - Maybe the source should only apply in ComputeCorr, comparaison a prevoir
			/*const float volu = float(double(MassPrec_M[p]) / double(rhopnew));
			float adens = float(LambdaMass * (RhopZero / rhopnew - 1));
			rhopnew = float(rhopnew + dt05 * adens);

			Massc_M[p] = float(double(MassPrec_M[p]) + dt05 * double(adens*volu));*/
			Velrhopc[p].w = rhopnew;
...
void JSphSolidCpu::ComputeSymplecticCorr_SigCst_M(double dt) {
	if (TShifting)ComputeSymplecticCorrT_SigCst_M<true>(dt);
	else         ComputeSymplecticCorrT_SigCst_M<false>(dt);
}

template<bool shift> void JSphSolidCpu::ComputeSymplecticCorrT_SigCst_M(double dt) {
	TmcStart(Timers, TMC_SuComputeStep);
...
#pragma omp parallel for schedule (static) if(npb>OMP_LIMIT_COMPUTESTEP)
	...
	for (int p = 0; p < npb; p++) {
		const double epsilon_rdot = (-double(Arc[p]) / double(Velrhopc[p].w))*dt;
		//const float rhopnew = float(double(VelrhopPrec[p].w) * (2. - epsilon_rdot) / (2. + epsilon_rdot));
		const float rhopnew = RhopZero - RhopZero /2.0f*float(TimeStep)*float(TimeStep);
		Velrhopc[p] = TFloat4(0, 0, 0, (rhopnew < RhopZero ? RhopZero : rhopnew));//-Avoid fluid particles being absorbed by boundary ones. | Evita q las boundary absorvan a las fluidas.
	}
...
#pragma omp parallel for schedule (static) if(np>OMP_LIMIT_COMPUTESTEP)
	...
	for (int p = npb; p < np; p++) {
		...
		//float rhopnew = float(double(VelrhopPrec[p].w) * (2. - epsilon_rdot) / (2. + epsilon_rdot));
		float rhopnew = RhopZero*exp(float(-0.5*TimeStep*TimeStep));
		...
		if (!WithFloating || CODE_IsFluid(Codec[p])) {//-Fluid Particles.
			...
			tmatrix3f GdVel = TMatrix3f(StrainDotc_M[p].xx, StrainDotc_M[p].xy, StrainDotc_M[p].xz
				...
				, StrainDotc_M[p].xz, StrainDotc_M[p].yz, StrainDotc_M[p].zz) + TMatrix3f(Spinc_M[p].xx, Spinc_M[p].xy, Spinc_M[p].xz
					, -Spinc_M[p].xy, Spinc_M[p].yy, Spinc_M[p].yz
					, -Spinc_M[p].xz, -Spinc_M[p].yz, Spinc_M[p].zz);
			/*tmatrix3f GdVel = TMatrix3f(float(TimeStep), 0, 0, 0, 0, 0, 0, 0, 0);*/
			NabVx_M[p] = GdVel.a11;
			...
			/*if ((Nstep == 200 || Nstep == 250 || Nstep == 300) && abs(Posc[p].y) < 0.05 && abs(Posc[p].z) < 0.05) {
				printf("CenterId %d Posx %.8f Vx %.8f Gvx %.8f (Part %d - Nsteps %d)\n", Idpc[p]
					, Posc[p].x, Velrhopc[p].x, GdVel.a11, Part, Nstep);
			}*/
		}
