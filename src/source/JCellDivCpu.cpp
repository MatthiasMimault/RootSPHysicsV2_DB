...
void JCellDivCpu::SetMemoryVSort(byte *vsort){
  ...
  VSortMatrix3f = (tmatrix3f*)VSort;	VSortBool = (bool*)VSort;
}
...
//==============================================================================
/// Reorder values of all particles (for type tmatrix3f)
/// Matthias
//==============================================================================
void JCellDivCpu::SortArray(tmatrix3f *vec) {
	const int n = int(Nptot);
	const int ini = (DivideFull ? 0 : int(NpbFinal));
...
#endif
	for (int p = ini; p < n; p++) {
		VSortMatrix3f[p] = vec[SortPart[p]];
	}
	memcpy(vec + ini, VSortMatrix3f + ini, sizeof(tmatrix3f)*(n - ini));
}

//==============================================================================
/// Matthias
/// Reorder values of all particles (for type bool)
//==============================================================================
void JCellDivCpu::SortArray(bool *vec) {
	const int n = int(Nptot);
	const int ini = (DivideFull ? 0 : int(NpbFinal));
#ifdef OMP_USE
#pragma omp parallel for schedule (static) if(n>OMP_LIMIT_COMPUTELIGHT)
#endif
	for (int p = ini; p<n; p++)VSortBool[p] = vec[SortPart[p]];
	memcpy(vec + ini, VSortBool + ini, sizeof(bool)*(n - ini));
}
