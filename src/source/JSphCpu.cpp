...
void JSphCpu::PreInteractionVars_Forces(TpInter tinter,unsigned np,unsigned npb){
  ...
  // Matthias
  if (JauGradvelc_M)memset(JauGradvelc_M + npb, 0, sizeof(tmatrix3f)*npf);  //JauGradvelc[]=(0,0,0,0,0,0).
...
/// Matthias
//==============================================================================
template<bool shift> void JSphCpu::ComputeEulerVarsFluid_M(tfloat4 *velrhop, double dt, tdouble3 *pos, unsigned *dcell, word *code)const
{
	const int pini = int(Npb), pfin = int(Np), npf = int(Np - Npb);
#ifdef _WITHOMP
...
/// Matthias
//==============================================================================
void JSphCpu::ComputeEuler_M(double dt) {
	TmcStart(Timers, TMC_SuComputeStep);
