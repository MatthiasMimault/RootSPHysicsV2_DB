...
/// Gencase and Real data. Matthias version 2019
//==============================================================================
void JPartsLoad4::LoadParticlesMixed_M(const std::string& casedir, const std::string& casename, unsigned partbegin, const std::string& casedirbegin) {
	const char met[] = "LoadParticles";
	//#Mixed #loadparticles
	Reset();
	PartBegin = partbegin;
	JPartDataBi4 pd;
	//#printf
	printf("Call of Mixed Loadparticle\n");
	...
	//load particles id & positions
	{
		...
		if (npok) {
			...
			// Position
			if (possingle) {
				for (unsigned p = 0; p < 1; p++)Pos[ntot + p] = ToTDouble3({ 2.0f,1.0f,0.0f });
			}
			else for (unsigned p = 0; p < 1; p++) Pos[ntot + p] = { 2.0,1.0,0.0 };
			// Idp
			Idp[ntot + 1] = npok;

			// Velocity & Density
			for (unsigned p = 0; p < 1; p++)VelRhop[ntot + p] = TFloat4(0, 0, 0, 1000);

			// Mass
			for (unsigned p = 0; p < 1; p++)Mass[ntot + p] = 1;
		}
		ntot += 1;
